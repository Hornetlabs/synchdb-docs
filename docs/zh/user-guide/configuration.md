...
| synchdb.dbz_snapshot_min_row_to_stream_results | integer | 0 | 在初始快照期间，Debezium 嵌入式引擎切换到流模式之前远程表应包含的最小行数。将其设置为 0 以始终切换到流模式 |
| synchdb.dbz_incremental_snapshot_chunk_size | integer | 2048 | 在增量快照期间，Debezium 嵌入式引擎为 SynchDB 生成的更改事件的最大数量 |
